...
int particles_out_of_area_number;
int *particles_out_of_area_cum_sum_array;

int *next_particle_in_area;
...
{
  int particles_out_of_area_cum_sum = 0;
  MPI_Scan(&particles_out_of_area_number, &particles_out_of_area_cum_sum, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);

  if (rank == size - 1) {
    nparticles = nparticles_at_start - particles_out_of_area_cum_sum;
  }

  MPI_Bcast(&nparticles, 1, MPI_INT, size - 1, MPI_COMM_WORLD);

  particles_out_of_area_cum_sum -= particles_out_of_area_number;

  MPI_Allgather(&particles_out_of_area_cum_sum,
                1,
                MPI_INT,
                particles_out_of_area_cum_sum_array,
                1,
                MPI_INT,
                MPI_COMM_WORLD);
  ...
  index_start_local = rank * nparticles_per_node + particles_out_of_area_cum_sum;
  index_end_local = (rank + 1) * nparticles_per_node + particles_out_of_area_number;
  if (rank == size - 1)
  {
   ...
   * displs - start position of each node reply in the receive buffer
   */
  int i;
  for (i = 0; i < size - 1; i++)
  {
    recvcounts[i] = nparticles_per_node +
                    particles_out_of_area_cum_sum_array[i + 1] - particles_out_of_area_cum_sum_array[i];
    displs[i] = i * nparticles_per_node + particles_out_of_area_cum_sum_array[i];
  }
  recvcounts[size - 1] = nparticles_at_start -
                         (size - 1) * nparticles_per_node - particles_out_of_area_cum_sum_array[size - 1];
...
void init(int argc, char **argv)
{
  ...
  particles_out_of_area_cum_sum_array = malloc(sizeof(int) * size);
  next_particle_in_area = malloc(sizeof(int) * nparticles);

  for (int i = 0; i < nparticles; i++) {
    next_particle_in_area[i] = i;
  }
...
void move_particle_without_insert_to_tree(particle_t *p, double step)
{
  ...
  p->node = NULL;
  if (is_particle_out_from_area(p, root)) {
    particles_out_of_area_number += 1;
  }
}
...
void move_all_particles_in_root_area(double step)
{
  for (int i = index_start_local; i < index_end_local; i++)
  {
    ...
    if (is_particle_out_from_area(p, root))
    {
      i = next_particle_in_area[i] - 1;

      continue;
    }

    move_particle_without_insert_to_tree(p, step);

    if (is_particle_out_from_area(p, root))
    {
      if (i == nparticles_at_start) {
        next_particle_in_area[i] = nparticles_at_start;
        continue;
      }

      next_particle_in_area[i] = next_particle_in_area[i + 1];

      for (int j = i - 1; j >= index_start_local; j--) {
        if (next_particle_in_area[j] != i)
          break;

        next_particle_in_area[j] = next_particle_in_area[i];
      }
    }
  }
}
...
void free_memory()
{
  ...
  free(recvcounts);
  free(displs);
  free(particles_out_of_area_cum_sum_array);
  free(next_particle_in_area);
